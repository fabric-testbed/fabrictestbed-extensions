...
class FabNet4ExtTest(unittest.TestCase):
    ...
    def test_1_fabnetv4_ext_create_slice(self):
        ...
        iface1 = node1.add_component(model='NIC_Basic', name=self.node1_nic_name).get_interfaces()[0]

        # Node2
        node2 = slice.add_node(name=self.node2_name, site=site2)
        iface2 = node2.add_component(model='NIC_Basic', name=self.node2_nic_name).get_interfaces()[0]

        # NetworkS
        net1 = slice.add_l3network(name=self.network1_name, interfaces=[iface1], type='IPv4Ext')
        net2 = slice.add_l3network(name=self.network2_name, interfaces=[iface2], type='IPv4Ext')
